...
namespace Improving.Highway.Data.Scope.DbContextScope
{
    ...
    {
        private readonly Dictionary<Type, IDomainContext<IDomain>> _initializedDbContexts;
        private readonly Dictionary<IDomainContext<IDomain>, DbContextTransaction> _transactions;
        ...
        internal Dictionary<Type, IDomainContext<IDomain>> InitializedDbContexts => _initializedDbContexts;
        ...
        public DbContextCollection(bool readOnly = false, IsolationLevel? isolationLevel = null, IDbContextFactory dbContextFactory = null)
        {
            ...
            _initializedDbContexts = new Dictionary<Type, IDomainContext<IDomain>>();
            _transactions = new Dictionary<IDomainContext<IDomain>, DbContextTransaction>();
        ...
        public IDomainContext<TDomain> Get<TDomain>() where TDomain : class, IDomain
        {
            ...
            var requestedType = typeof(IDomainContext<TDomain>);
            ...
            if (!_initializedDbContexts.ContainsKey(requestedType))
            {
                ...
                if (_isolationLevel.HasValue)
                {
                    ...
                    if (efDbContext != null)
                    {
                        ...
                        var tran = efDbContext.Database.BeginTransaction(_isolationLevel.Value);
                        _transactions.Add((IDomainContext<IDomain>) dbContext, tran);
                    }
                }
            }
        ...
        public async Task<int> CommitAsync(CancellationToken cancelToken)
        {
            ...
            foreach (var dbContext in _initializedDbContexts.Values)
            {
                try
                {
                    if (!_readOnly)
                    {
                        c += await dbContext.CommitAsync().ConfigureAwait(false);
                    }
